...
class npy2obj:
    def __init__(self, npy_path, sample_idx, rep_idx, device=0, cuda=True):
        ...
        # pdb.set_trace()
        self.rot2vert = Rotation2vertices(device='cpu') ## added
        self.faces = self.rot2vert.smplx_model.faces # added
        ...
        #                              vertstrans=True)
        self.vertices, _ = self.rot2vert(torch.tensor(self.motions['motion']), mask=None,  ## added
                                     pose_rep='smplx', translation=False, glob=True,
                                     jointstype='vertices',
                                     # jointstype='smpl',  # for joint locations
                                     vertstrans=True)
    ...
    def get_vertices(self, sample_i, frame_i):
        # print("==============> mesh: ", self.vertices.shape)
        print("==============> mesh: ",self.vertices[sample_i, :, :, frame_i].squeeze().shape, "==========>", len(self.vertices[sample_i, :, :, frame_i].squeeze().tolist()))
