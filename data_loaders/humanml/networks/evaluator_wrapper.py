...
def build_evaluators(opt):
    ...
    checkpoint = torch.load(pjoin(opt['checkpoints_dir'], ckpt_dir, 'diffusion.35k', 'model', 'finest.tar'),
                            map_location=opt['device'])
    del checkpoint['text_encoder']['pos_emb._float_tensor']
    movement_enc.load_state_dict(checkpoint['movement_encoder'])
    text_enc.load_state_dict(checkpoint['text_encoder'])
    motion_enc.load_state_dict(checkpoint['motion_encoder'])
    print('Loading Evaluation Model Wrapper (Epoch %d) Completed!!' % (checkpoint['epoch']))
...
class EvaluatorMDMWrapper(object):
    ...
    def __init__(self, dataset_name, device):
        opt = {
            ...
            'dim_pose': 263 if dataset_name == 'humanml' else 318,
    ...
    def get_co_embeddings(self, word_embs, cap_lens, motions, m_lens):
        ...
        with torch.no_grad():
            ...
            align_idx = np.argsort(m_lens.data.tolist())[::-1].copy()
            text_align_idx = np.argsort(cap_lens.data.tolist())[::-1].copy()
            motions = motions[align_idx]
            m_lens = m_lens[align_idx]
            cap_lens = cap_lens[text_align_idx]
