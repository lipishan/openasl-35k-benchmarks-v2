...
def main(args=None):
    ...
    max_frames = 400 if args.dataset in ['kit', 'humanml', 'openasl'] else 60
    fps = 12.5 if args.dataset == 'kit' else 25
    # pdb.set_trace()
    ...
    for rep_i in range(args.num_repetitions):
        ...
        original_sample = sample # this is the 6d/3d rotation pose generated from diffusion
        # pdb.set_trace()
        mask = model_kwargs['y']['mask'][:, :, :, :n_frames]
        rot2xyz_pose_rep = 'xyz' if model.data_rep in ['xyz', 'hml_vec'] else model.data_rep
        rot2xyz_mask = None if rot2xyz_pose_rep == 'xyz' else mask.reshape(args.batch_size, n_frames).bool()
        sample = model.rot2xyz(x=sample, mask=rot2xyz_mask, pose_rep=rot2xyz_pose_rep, glob=True, translation=False,
        ...
        pdb.set_trace()
        all_original_pose.append(original_sample.cpu().numpy())
        all_motions.append(sample.cpu().numpy()) # this is joints 3d regressed from smplx from the vertices, used for skeleton 
    ...
    all_original_pose = np.concatenate(all_original_pose, axis=0)
    all_original_pose = all_original_pose[:total_num_samples]  # [bs, njoints, 6, seqlen]
    ...
    # Save 3d/6d rotation pose
    npy_path = os.path.join(out_path, 'pose_results.npy')
    print(f"saving results file to [{npy_path}]")
    np.save(npy_path,
            {'motion': all_original_pose, 'text': all_text, 'lengths': all_lengths,
             'num_samples': args.num_samples, 'num_repetitions': args.num_repetitions})
    
    # save skeleton data regressed from mesh
    ...
    skeleton = paramUtil.kit_kinematic_chain if args.dataset == 'kit' else paramUtil.asl_kinematic_chain
...
def load_dataset(args, max_frames, n_frames):
    data = get_dataset_loader(name=args.dataset,
                              ...
                              split='train',
                              hml_mode='text_only',  # We need to sample a prefix from the dataset
